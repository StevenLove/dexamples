...
fn get_key_from_todo_id(id: u32) -> String {
    //  for breaking behavior, return the same value regardless of id
    // "x".to_string()

    // for proper behavior, return a string representation of the id
    id.to_string()
}
        ...
        section { class: "todoapp",
            ...
            section { class: "main",
                if !todos.read().is_empty() {
                    ...
                    label { r#for: "toggle-all", "toggle all" }
                }
                ...
                ul { class: "todo-list",
                    for id in filtered_todos() {
                        TodoEntry { key: "{get_key_from_todo_id(id)}", id, todos }
                    }
                }
...
fn TodoEntry(mut todos: Signal<HashMap<u32, TodoItem>>, id: u32) -> Element {
    ...
    rsx! {
        li {
            ...
            div { class: "view", background: color,
                input {
                    class: "toggle",
                    r#type: "checkbox",
                    id: "cbg-{id}",
                ...
                ]
                {
                    li {
                        a {
                            ...
                            onclick: move |evt| { filter.set(state) },
                            {state_text}
                        }
                    }
                }
